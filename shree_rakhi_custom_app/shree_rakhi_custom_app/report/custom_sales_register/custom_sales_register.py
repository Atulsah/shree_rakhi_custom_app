...
def get_data(filters):
	return frappe.db.sql("""select name, customer, customer_name, place_of_supply, total_qty, custom_number_of_carton, \
	    base_total, additional_discount_percentage, discount_amount, rounded_total, custom_bos_transporter from `tabSales Invoice` \
		WHERE posting_date BETWEEN %(from_date)s and %(to_date)s and docstatus=1 {itm_conditions} order by name \
